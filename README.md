...
- 诗词、不可变来源版本、完整字段快照的 ContentVersion、指令版本、需求卡、方向方案与审计事件；
...
- 数据质量问题可直接钻取到诗词内容或来源修订；内容修订会创建新 ContentVersion、冻结对应来源版本，并让当前需求与方向退出新排产但保留历史；
- 内容批量审批逐诗执行并返回部分失败；存在未结束生成任务时禁止修改上游内容，避免在途任务与内容快照漂移；
...
- 异常中心把失败、阻塞、QC、预算、导出与卡住任务投影为逐条生产记录，提供错误码、责任角色、建议动作、筛选和对应工作台跳转；
- 诗词阻塞持久化记录原因、错误码、责任角色、建议动作和发生时间；恢复成功后自动退出异常队列，本机路径与密钥在异常摘要中脱敏；
- 批量生成结构化需求卡；
- 服务端权威需求看板将全部诗词投影到待生成、生成中、待审核、已通过、已退回和阻塞六个阶段，并给出当前责任角色与操作门禁；
- 需求看板支持按作者、题材、阶段、历史风险、低置信度、流程阻塞、责任人和关键词组合筛选，卡片直接展示主体、环境、意象、风险与更新时间；
...
- 需求卡完整侧栏可版本化修订题材、情绪、环境、主体、意象、构图、必须项、禁用项、历史风险和不确定点；诗意证据与置信度保持只读，批准后全部字段冻结；
- 需求卡字段锁定、未锁字段重算、批量批准和带原因退回；
...
- 内容质量钻取、完整 ContentVersion 快照、内容修订历史、下游失效、活动任务锁和批量审批部分失败；
- 需求生成、修订、批准和退回；
...
- 服务中断后的结果未知保护与显式确认重试；
- 结构化阻塞责任、跨任务 / QC / 批次 / 版本运行的异常投影、筛选下钻、恢复自动清除和敏感摘要脱敏；
